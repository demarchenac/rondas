...
function InfographicPreview({ uri, imageAspect, visible, onShare, onClose }: InfographicPreviewProps) {
  ...
  const dismissPreview = useCallback(() => { onClose(); }, [onClose]);

  const panGesture = Gesture.Pan()
    .onEnd((e) => {
      if (e.translationY > DISMISS_THRESHOLD) {
        runOnJS(dismissPreview)();
      }
    });

  // Tap gesture on backdrop — blocks native gestures from reaching the modal
  ...
  return (
    <View className="absolute inset-0" style={{ zIndex: 100 }}>
      {/* Backdrop — GestureDetector blocks native modal gesture */}
      ...
      {/* Sheet — Pan gesture intercepts swipe before modal gets it */}
