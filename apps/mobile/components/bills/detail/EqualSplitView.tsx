  ...
  return (
    <View className="my-1 border-l-[3px] border-l-primary py-3 pl-3 pr-1">
      ...
      <Input
        ...
        className={`mb-3 rounded-lg border-border bg-card text-lg font-semibold ${Platform.OS === 'ios' ? 'h-10' : ''}`}
      ...
      <View className="mb-3 flex-row gap-3">
        <View className="w-20">
          <Text className="mb-1 text-sm font-semibold uppercase tracking-wider text-muted-foreground">
            {t.scan_qty}
          </Text>
          <Input
            value={quantity}
            ...
            className={`rounded-lg border-border bg-card text-center text-base ${Platform.OS === 'ios' ? 'h-10' : ''}`}
          />
        </View>
        <View className="flex-1">
          ...
          <CurrencyInput
            ...
            className={`rounded-lg border border-border bg-card px-3 text-base ${Platform.OS === 'ios' ? 'h-10' : ''}`}
          />
        </View>
      </View>
      ...
      <View className="flex-row items-center justify-between">
        ...
        <View className="flex-row gap-2">
          ...
          >
            <Text className="text-base font-semibold text-muted-foreground">{t.cancel}</Text>
          </Pressable>
          <Pressable
            onPress={() => {
          ...
          >
            <Text className="text-base font-semibold text-primary-foreground">{t.done}</Text>
          </Pressable>
        </View>
      </View>
    </View>
    ...
    <View className="gap-4">
      ...
      <Animated.View entering={FadeInDown.delay(100).duration(400)} className="rounded-2xl border border-border bg-card p-5">
        ...
        <View className="mt-3 items-center">
          ...
          <Text className="text-base text-muted-foreground">{t.bill_equalPerPerson}</Text>
          {remainder > 0 && (
            ...
            <SwipeableRow key={itemId} onDelete={() => onRemoveItem(itemId)} bgClassName="bg-card">
              ...
              >
                <View className="flex-1 mr-2">
                  <Text className="text-base text-foreground" numberOfLines={1}>{item.name || t.scan_itemName}</Text>
                  {item.quantity > 1 && (
